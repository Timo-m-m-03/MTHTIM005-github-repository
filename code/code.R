...
plant <- read.csv("data/plant list.csv")
 


leaf <- read.csv("data/leaf traits.csv")                                       %>% rename('species' = 'Species', 'speciman ID' = 'Number', 'average leaf length (cm)' = 'Av..leaf.length', 'average leaf width (cm)' = 'Av..leaf.width', 'average leaf thickness (mm)' = 'Av..leaf.thickness', 'single leaf surface area (cm2)' = 'Leaf.surface.area.per.leaf', 'fresh leaf weight (g)' = 'Leaf.fresh.weight', 'dry leaf weight (g)' = 'Leaf.dry.weight', 'specific leaf area (m2 g DW-1)' = 'Specific.leaf.area', 'leaf water content (%)' = 'Leaf.water.content') %>% slice(-1) %>% select (-X, -X.1, -X.2)                                      %>% mutate(`average leaf length (cm)` = as.character(`average leaf length (cm)`), `average leaf width (cm)` = as.character(`average leaf width (cm)`) ,         `average leaf thickness (mm)` = as.character(`average leaf thickness (mm)`),  `single leaf surface area (cm2)` = as.character(`single leaf surface area (cm2)`) ,`fresh leaf weight (g)` = as.character(`fresh leaf weight (g)`),         `dry leaf weight (g)` = as.character(`dry leaf weight (g)`),                   `specific leaf area (m2 g DW-1)` = as.character(`specific leaf area (m2 g DW-1)`), `leaf water content (%)` = as.character(`leaf water content (%)`))        %>% mutate(`average leaf length (cm)` = as.numeric(`average leaf length (cm)`),    `average leaf width (cm)` = as.numeric(`average leaf width (cm)`),       `average leaf width (cm)` = as.numeric(`average leaf width (cm)`),         `average leaf thickness (mm)` = as.numeric(`average leaf thickness (mm)`),  `single leaf surface area (cm2)` = as.numeric(`single leaf surface area (cm2)`) ,`fresh leaf weight (g)` = as.numeric(`fresh leaf weight (g)`),              `dry leaf weight (g)` = as.numeric(`dry leaf weight (g)`),                   `specific leaf area (m2 g DW-1)` = as.numeric(`specific leaf area (m2 g DW-1)`), `leaf water content (%)` = as.numeric(`leaf water content (%)`))            %>% mutate(`species` = as.factor(`species`))

 

flammability <- read.csv("data/flamability traits.csv")                        %>% rename('species' = 'Species','speciman ID' = 'Number', 'leaf ID' = 'Leaf.no.', 'fresh leaf ignitability (s)' = 'Fresh.leaves', 'fresh leaf sustainability (s)' = 'X', 'dry leaf ignitability (s)' = 'Dry.leaves', 'dry leaf sustainability (s)' = 'X.1')                                                   %>% slice(-1)                                                                  %>% fill("speciman ID", .direction = "down")                                   %>% mutate(`fresh leaf ignitability (s)` = as.character(`fresh leaf ignitability (s)`), `fresh leaf sustainability (s)` = as.character(`fresh leaf sustainability (s)`),`dry leaf ignitability (s)` = as.character(`dry leaf ignitability (s)`),`dry leaf sustainability (s)` = as.character(`dry leaf sustainability (s)`))                                                          %>% mutate(`fresh leaf ignitability (s)` = as.numeric(`fresh leaf ignitability (s)`), `fresh leaf sustainability (s)` = as.numeric(`fresh leaf sustainability (s)`),`dry leaf ignitability (s)` = as.numeric(`dry leaf ignitability (s)`),`dry leaf sustainability (s)` = as.numeric(`dry leaf sustainability (s)`))   %>% mutate(`species` = as.factor(`species`))



combustibility <- read.csv("data/combustibility traits.csv")                   %>% rename ('species' = 'Species','speciman ID' = 'Number','combustibility (mg DW s-1)' = 'Combustibility')                                                   %>% slice(-1)                                                                  %>% mutate(`combustibility (mg DW s-1)` = as.character(`combustibility (mg DW s-1)`))                                                                         %>% mutate(`combustibility (mg DW s-1)` = as.numeric(`combustibility (mg DW s-1)`))                                                                           %>% mutate(`species` = as.factor(`species`))
